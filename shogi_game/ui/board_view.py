...
class BoardView(tk.Frame):
    ...
    def __init__(self, parent, board, config, on_move_callback):
        ...
        # キャンバスを作成
        canvas_width = self.layout_metrics.canvas_width
        canvas_height = self.layout_metrics.canvas_height
    ...
    def _get_cell_rect(self, row, col):
        """指定マスの矩形を返す"""
        return get_cell_rect(self.layout_metrics, row, col)

    def _get_cell_center(self, row, col):
        """指定マスの中心座標を返す"""
        return get_cell_center(self.layout_metrics, row, col)
