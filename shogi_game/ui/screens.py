...
class GameScreen(tk.Frame):
    ...
    def __init__(self, parent, app, game_mode, difficulty, save_data=None):
        ...
        self.game_mode = game_mode
        self.difficulty = difficulty
        self.tsume_manager = TsumeManager() if game_mode == 'tsume' else None
        ...
        self.controller = GameFlowController(
            ...
            board=self.board,
            engine=self.engine,
            tsume_manager=self.tsume_manager,
    ...
    def _get_turn_text(self):
        """手番テキストを取得"""
        return self.controller.get_turn_text()
    ...
    def _apply_update(self, update):
        ...
        if update.turn_text is not None:
            self.turn_label.config(text=update.turn_text, fg=update.turn_color)

        if update.input_enabled is not None:
            self.board_view.set_enabled(update.input_enabled)

        if update.status_level:
            if update.status_level == 'info':
                messagebox.showinfo(update.status_title, update.status_message)
            elif update.status_level == 'warning':
                messagebox.showwarning(update.status_title, update.status_message)
            elif update.status_level == 'error':
                messagebox.showerror(update.status_title, update.status_message)

        if update.result_type:
            self.app.show_result_screen(update.result_type, self.game_mode, self.difficulty)
            return
    ...
    def get_save_data(self):
        """保存データを取得"""
        return self.controller.get_save_data()
